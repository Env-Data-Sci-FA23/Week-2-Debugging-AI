...
1.  Changed r setup code to echo = FALSE

2.  Need to call in the tidyverse package and palmer penguins package - ran library(tidyverse) and library(palmerpenguins)

3.  in geom_point, colors were not assigned to species - fixed with color = species, inside aes()

4.  Adelie data frame was not created, so I added adelie \<- to the code

5.  Changed the spelling of "Adelie" in the filter
