...
1.  Need to call in the tidyverse packages through library and palmer penguins function.

2.  Change the initial code script to echo = FALSE to hide code.

3.  Geom point is missing assigning colors to each species (color = species).

4.  When using filter, there is a spelling error (Adelie).

5.  We need to assign a name to the filtered adelie data.
