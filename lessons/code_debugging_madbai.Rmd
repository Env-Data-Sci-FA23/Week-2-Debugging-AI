...
1.  Must load in {palmerpenguins} and {tidyverse} packages with the library() function

2.   To visualize the relationship between body mass and bill length *among* species you need to set color = species

3.  To create a new data frame you must use adelie \<-

4.  When filtering for species change "Adelei" to correct spelling "Adelie"

5.  To mirror HTML, in knitr::opts_chunk\$set(echo = TRUE, message = FALSE), set echo = False
