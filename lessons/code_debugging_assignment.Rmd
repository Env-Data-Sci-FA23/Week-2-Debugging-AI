...
## The Assignment: This assignment is worth 10 points. 5 points for finding the errors, 5 points for successful submission as a PR.
...
There are a total of **5 fixes** you need to make to the workflow below. To complete this assignment you must ***1)*** Identify the 5 errors and specify what they are below (*4 pts each*) and ***2)*** fix the errors and render this entire .Rmd as an HTML doc where the content below matches that of the `code_debugging_output.html` (*5 pts*).

**List here the 5 errors you had to fix in this workflow:**

1.  In first chunk: warning: data set 'penguins' not found 

2.  In fourth chunk: Adelie referenced incorrectly - no data in tibble 

3.  In fifth chunk: object 'Adelie' not found 

4.  Tidyverse package not loaded 

5.  In fourth chunk Adelie species not filtered into separate data set for use in ggplot for next chunk. 
